...
if [ "$CONFIG_FOR" = "16GB" ]; then
  ...
  vm_slave_cpu[1]=1  # Controller
  vm_slave_cpu[2]=2  # Compute + Cinder
  vm_slave_cpu[3]=1  # ElasticSearch + Kibana
  vm_slave_cpu[4]=1  # InfluxDB + Grafana
else
  # Section for custom configuration
  vm_slave_cpu_default=1
...
if [ "$CONFIG_FOR" = "16GB" ]; then
  ...
  vm_slave_memory_mb[1]=2048  # Controller
  vm_slave_memory_mb[2]=2048  # Compute + Cinder
  vm_slave_memory_mb[3]=1024  # ElasticSearch + Kibana
  vm_slave_memory_mb[4]=1024  # InfluxDB + Grafana
else
  # Section for custom configuration
  vm_slave_memory_default=1024
